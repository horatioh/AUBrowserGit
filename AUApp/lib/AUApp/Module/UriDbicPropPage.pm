package AUApp::Module::UriDbicPropPage;
...
sub Controller {
  ...
  if ($uri)  {
	  my $forward_uri = $uri;
	  my $search_str = $self->URIFilePath;
	  my $replace_str = $self->URIBasePath;
	  
	  $forward_uri =~ s/$search_str/$replace_str/ ;
	
	  $self->prepare_controller(@args);
	
	  # dispatch the request to the appropriate handler
#$DB::single=1;
	  $c->log->debug('UriDbicPropPage Module Controller--> ');  
	  $c->log->debug("URI--> $uri");
	  $c->log->debug("Go to URI--> $forward_uri");

	  # find action for uri and go to action	
