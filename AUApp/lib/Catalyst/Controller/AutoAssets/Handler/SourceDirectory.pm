package Catalyst::Controller::AutoAssets::Handler::SourceDirectory;
use strict;
use warnings;
...
sub _set_file_response {
  ...
  # Added PG
  if ($content_type =~ m/\Atext\/as400\z/)  {
	  $c->log->debug("Show as400 file");
	  my @linearray = read_file($file, chomp => 1 );
	  my $i=1;
	  my $lines = "File: $file\n" . "000000:|---+----1----+----2----+----3----+----4----+----5----+----6----+----7----+----8\n";
	  foreach (@linearray )  {
	  	$lines = $lines . sprintf('%06u%s%s',$i, ':', $_);
	  	$i++;
	  }
#DB::single=1;
#	  $c->log->debug("SourceDirectory: _set_file_response-> Content-Type: $content_type");
	  $c->log->debug("Response Header-> " . Dumper($c->response->headers()));
	  return $c->response->body( $lines );
  }
  else { 
	  $c->log->debug("Show non as400 file");
	  my $f= $file->openr;
	  binmode $f;
...
#$DB::single=1;
  ...
  if ( $full_path =~ /.*\.(\S{1,})$/xms ) {
    ...
    my $type = $self->MimeTypes->mimeTypeOf( $ext );
   	if ($ext =~ m/\A(RPG|CLP|PF|LF|DSPF|PRTF)\z/ ) {
   		$type = 'text/as400';
  	}
    $c->log->debug("_ext_to_type-> Ext: $ext Type: $type");
    if ( $type ) {
